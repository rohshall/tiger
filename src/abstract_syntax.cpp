...
A_Program::A_Program(const yy::location& _loc, A_exp* _exp) : AstNode(_loc), exp(_exp) {}
...
A_fieldVar::A_fieldVar(const yy::location& _loc, A_var* _var, const std::string& _id)
    : var(_var), id(std::move(_id)) {
  loc = _loc;
  kind = Kind::A_fieldVar;
}

A_subscriptVar::A_subscriptVar(const yy::location& _loc, A_var* _var, A_exp* _exp)
    : var(_var), exp(_exp) {
  loc = _loc;
...
A_callExp::A_callExp(const yy::location& _loc, const std::string& _id, A_expList* _args)
...
A_opExp::A_opExp(const yy::location& _loc, A_oper _op, A_exp* _left, A_exp* _right)
...
A_arrayExp::A_arrayExp(const yy::location& _loc, const std::string& _typ, A_exp* _size,
           A_exp* _init)
...
A_ifExp::A_ifExp(const yy::location& _loc, A_exp* _test, A_exp* _tbody, A_exp* _fbody)
...
A_forExp::A_forExp(const yy::location& _loc,const std::string& _id, A_exp* _low, A_exp* _high, A_exp* _body)
...
A_typeDec::A_typeDec(const yy::location& _loc, const std::string& _typ, A_ty* _ty)
    : type_id(std::move(_typ)), ty(_ty) {
  loc = _loc;
  kind = Kind::A_typeDec;
}

A_functionDec::A_functionDec(const yy::location& _loc, const std::string& _id, A_fieldList* _params,
...
A_nameTy::A_nameTy(const yy::location& _loc, const std::string& _id) : id(std::move(_id)) {
  loc = _loc;
  kind = Kind::A_nameTy;
}

A_recordTy::A_recordTy(const yy::location& _loc, A_fieldList* _record) : record(_record) {
  loc = _loc;
...
A_fieldList::A_fieldList(const yy::location& _loc, A_field* _head, A_fieldList* _tail)
...
A_efield::A_efield(const yy::location& _loc, const std::string& _id, A_exp* _exp)
    : id(std::move(_id)), exp(_exp) {
  loc = _loc;
}

A_efieldList::A_efieldList(const yy::location& _loc, A_efield* _head, A_efieldList* _tail)
    : head(_head), tail(_tail) {
  loc = _loc;
}
