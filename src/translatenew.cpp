...
IRNode* A_Program::translate(DeclarationTable& table) {
    fc.openFrame();
    IRNode* root = exp->translate(table);
    fc.closeFrame();
    return root;
//  exp->semanticCheck(table);
}

IRNode* A_simpleVar::translate(DeclarationTable& table) {
...
IRNode* A_fieldVar::translate(DeclarationTable& table) {
  var->semanticCheck(table);
  A_typeDec* dec = table.retrieveType(var->type);
  if (dec == nullptr) return NULL;
...
IRNode* A_subscriptVar::translate(DeclarationTable& table) {
  var->semanticCheck(table);
  exp->semanticCheck(table);
  A_typeDec* dec = table.retrieveType(var->type);
  if (dec == nullptr) return NULL;
...
IRNode* A_varExp::translate(DeclarationTable& table) {
  var->semanticCheck(table);
  this->type = var->type;
}

IRNode* A_nilExp::translate(DeclarationTable& table) { this->type = "nil"; }

IRNode* A_intExp::translate(DeclarationTable& table) { this->type = "int"; }

IRNode* A_stringExp::translate(DeclarationTable& table) {
  this->type = "string";
}

IRNode* A_callExp::translate(DeclarationTable& table) {
...
IRNode* A_opExp::translate(DeclarationTable& table) {
...
IRNode* A_recordExp::translate(DeclarationTable& table) {
...
IRNode* A_seqExp::translate(DeclarationTable& table) {
...
IRNode* A_assignExp::translate(DeclarationTable& table) {
  var->semanticCheck(table);
  exp->semanticCheck(table);
  if (var->type == "void") {
...
IRNode* A_ifExp::translate(DeclarationTable& table) {
...
IRNode* A_whileExp::translate(DeclarationTable& table) {
  test->semanticCheck(table);
  if (test->type != "int") {
...
IRNode* A_forExp::translate(DeclarationTable& table) {
...
IRNode* A_breakExp::translate(DeclarationTable& table) { return NULL; }

IRNode* A_letExp::translate(DeclarationTable& table) {
  table.beginScope();
  T_stmList* decRoot = new T_stmList(NULL, NULL);
  T_stmList* decCurList = decRoot;
  A_decList* cur_dec = decs.get();
  while (cur_dec != nullptr) {
      T_stm* cur_dect = IRNode2T_stm(cur_dec->head->translate(table));
      if(cur_dect == NULL)
...
IRNode* A_arrayExp::translate(DeclarationTable& table) {
  A_typeDec* dec = table.retrieveType(type_id);
  if (dec == nullptr) {
  ...
  T_exp* sizeRoot = IRnode2T_exp(size->translate(table));
//  size->semanticCheck(table);
  if (size->type != "int") {
    cat_driver::error(size->loc, "A_arrayExp: int type required");
  }
  T_exp* initRoot = IRnode2T_exp(init->translate(table));
...
IRNode* A_varDec::translate(DeclarationTable& table) {
//  init->semanticCheck(table);
  T_exp* rootInit = IRnode2T_exp(init->translate(table));
  
  if(rootInit == NULL)
    std::cout << "Vardec Wrong" << std::endl;
...
IRNode* A_typeDec::translate(DeclarationTable& table) {
...
IRNode* A_functionDec::translate(DeclarationTable& table) {
...
IRNode* A_nameTy::translate(DeclarationTable& table) {
...
IRNode* A_recordTy::translate(DeclarationTable& table) {
...
IRNode* A_arrayTy::translate(DeclarationTable& table) {
  A_typeDec* dec = table.retrieveType(id);
  if (dec == nullptr) {
